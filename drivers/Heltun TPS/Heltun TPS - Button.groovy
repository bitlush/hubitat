...
metadata {
    definition (name: "Heltun TPS - Button", namespace: "bitlush", author: "Keith Wood") {
        capability "Actuator"
        capability "Switch"
...
def push() {
    push(getButtonNumber())
}

def push(buttonNumber, buttonType = "digital") {
    logMessage("debug", "push(${buttonNumber},${buttonType})")
    
    parse([[name: "pushed", value: 1, type: buttonType, isStateChange: true]])
...
def getButtonNumber() {
    return device?.deviceNetworkId?.split('-')[1] as Integer
}

def hold() {
    hold(getButtonNumber())
}

def hold(buttonNumber, buttonType = 'digital') {
    logMessage("debug", "hold(${buttonNumber},${buttonType})")
    
    parse([[name: "held", value: 1, type: buttonType, isStateChange: true]])
}
